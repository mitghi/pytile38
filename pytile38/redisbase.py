from .base import (
    EndPointConnectionMeta,
    EndPointMeta,
    EndPointProtocolMeta,
    NotIMplementedError,
    isistance,
)
...
class RedisEndPoint(EndPointMeta):
    ...
    # TODO fix noqa C901, CCR001
    def _command_builder(self, command=None, schemestr=None, instance=None, **kwargs):  # noqa: C901, CCR001
        command_args = []
        cmds = []
        try:
            ...
            raise KeyError(
                "Invalid command or arguments ( command={}, scheme={} )".format(command or "", schemestr or "")
            )
        ...
        for cmdarg in cmds:
            ...
            if isinstance(cmdarg, Optional):
            ...
            elif isinstance(cmdarg, (Conditional, OptionalCondition)):  # noqa: R507
                iscond = isinstance(cmdarg, Conditional)
                cmdvalue = kwargs.get(cmdkey, getattr(instance, cmdkey, None))
                if cmdvalue is None:
                    if iscond:
                        raise KeyError(
                            "Invalid command or arguments ( command={}, scheme={} )".format(
                                command or "", schemestr or ""
                            )
                        )
                    if not iscond:
                        continue
                condkey = "".join(
                    [
                        schemestr,
                        ":",
                        str(kwargs.get(cmdkey, getattr(instance, cmdkey, ""))),
                    ]
                )
                condresult = None
                ...
                if iscond and condresult is None:
                    raise KeyError(
                        "Invalid command or arguments ( command={}, scheme={} )".format(command or "", schemestr or "")
                    )
                elif not iscond and condresult is None:
                    continue
    ...
    def command_builder(self, command=None, scheme=None, instance=None, **kwargs):
        ...
        for k, v in statement:
            if k == "fields":
                newv = list(map(lambda x: ["field"] + list(x), v))  # noqa: C417
                for nv in newv:
                    result += nv
                continue
            result.append(v)
